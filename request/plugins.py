        ...
        for module_path in settings.PLUGINS:
            try:
                ...
                raise exceptions.ImproperlyConfigured(
                    "{} isn't a plugin".format(module_path)
                )
            ...
            try:
                ...
                raise exceptions.ImproperlyConfigured(
                    'Error importing plugin {}: "{}"'.format(plugin, e)
                )
            ...
            try:
                ...
                raise exceptions.ImproperlyConfigured(
                    'Plugin "{}" does not define a "{}" class'.format(
                        plugin,
                        plugin_classname,
                    )
                )
...
class Plugin:
    ...
    def render(self):
        templates = [
            "request/plugins/{}.html".format(self.__class__.__name__.lower()),
...
class TopSearchPhrases(Plugin):
    def template_context(self):
        return {
            "phrases": set_count(
                self.qs.search().only("referer").attr_list("keywords")
            )[:10]
        }
